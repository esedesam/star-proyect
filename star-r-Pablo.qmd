...
p_load(AER, ggplot2, ggthemes, dplyr, tidyr, knitr, fastDummies, MatchIt, cobalt)
...
```{r preprocesado-dfs}
# Eliminar los registros con NAs
...
```{r df-kindergarten}
StarkDf_encoded_grouped
```

```{r df-1}
Star1Df_encoded_grouped
```

```{r df-2}
Star2Df_encoded_grouped
```


```{r df-3}
Star3Df_encoded_grouped
```
...
```{r match-full}
star1_match <- Star1Df[!(Star1Df$star1 == "regular+aide"),]
star1_match$star1 <- factor(star1_match$star1, levels = c("regular", "small"), labels = c(0, 1))
m.ps.nn.1 <- matchit(star1 ~ experience1 + lunch1 + school1 + gender1 +  ethnicity1,
                  method="full",
                  distance="glm",
                  data=star1_match,
                  estimand = "ATE")

stark_match <- StarkDf[!(StarkDf$stark == "regular+aide"),]
stark_match$stark <- factor(stark_match$stark, levels = c("regular", "small"), labels = c(0, 1))
m.ps.nn.k <- matchit(stark ~ experiencek + lunchk + schoolk + genderk +  ethnicityk,
                  method="full",
                  distance="glm",
                  data=stark_match,
                  estimand = "ATE")

star2_match <- Star2Df[!(Star2Df$star2 == "regular+aide"),]
star2_match$star2 <- factor(star2_match$star2, levels = c("regular", "small"), labels = c(0, 1))
m.ps.nn.2 <- matchit(star2 ~ experience2 + lunch2 + school2 + gender2 +  ethnicity2,
                  method="full",
                  distance="glm",
                  data=star2_match,
                  estimand = "ATE")

star3_match <- Star3Df[!(Star3Df$star3 == "regular+aide"),]
star3_match$star3 <- factor(star3_match$star3, levels = c("regular", "small"), labels = c(0, 1))
m.ps.nn.3 <- matchit(star3 ~ experience3 + lunch3 + school3 + gender3 +  ethnicity3,
                  method="full",
                  distance="glm",
                  data=star3_match,
                  estimand = "ATE")
...
```{r love-plot}
love.plot(m.ps.nn.k,stats = c("m","var","ks"), 
...
love.plot(m.ps.nn.2,stats = c("m","var","ks"), 
          abs = TRUE,thresholds=c(m=0.1,ks=0.05,var=2),
          stars = "std")
love.plot(m.ps.nn.3,stats = c("m","var","ks"), 
          abs = TRUE,thresholds=c(m=0.1,ks=0.05,var=2),
          stars = "std")

```

```{r datos-match-medias}
datos_match_k <- match.data(m.ps.nn.k)
datos_match_1 <- match.data(m.ps.nn.1)
datos_match_2 <- match.data(m.ps.nn.2)
datos_match_3 <- match.data(m.ps.nn.3)


datos_match_k %>% 
  group_by(stark) %>% 
  summarise(weighted.mean(notak, weights))

datos_match_1 %>% 
  group_by(star1) %>% 
  summarise(weighted.mean(nota1, weights))

datos_match_2 %>% 
  group_by(star2) %>% 
  summarise(weighted.mean(nota2, weights))

datos_match_3 %>% 
  group_by(star3) %>% 
  summarise(weighted.mean(nota3, weights))
```
...
# Cargar los paquetes necesarios
p_load(AER, ggplot2, ggthemes, dplyr, tidyr, knitr, stargazer, sandwich, GGally, stringr, vcd)
...
*Añadir referencias*
>>>>>>> a5d2b36c81d5d378478de89bebae6a21577c4e4e:star-r.qmd
