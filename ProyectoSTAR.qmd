---
title: "Proyecto STAR"
subtitle: "Inferencia Causal y Aprendizaje Máquina"
...
p_load(AER, ggplot2, ggthemes, dplyr, tidyr, knitr, stargazer, sandwich, GGally, stringr, vcd, ggdag, kableExtra, reticulate, MatchIt, cobalt, marginaleffects, forcats)
...
El enlace para ver el proceso de creación del proyecto se encuentra en el siguiente [repositorio de GitHub](https://github.com/esedesam/star-proyect.git).
...
Como era de esperar, hay una correlación bastante notable entre las puntuaciones de lectura y matemáticas en todos los cursos. Quien es bueno en una materia, es bueno en la otra. El resto de correlaciones entre estas variables mostradas no suelen ser muy altas, aunque sí son en algunos casos significativamente distintas de 0. Por ejemplo, resulta curioso observar cómo hay cierta correlación negativa entre las puntuaciones y el `birth_year` en *KinderGarten* (quizás la diferencia de edad se note más en las edades tempranas). Ocurre lo contrario para el resto de cursos, habiendo una correlación positiva.
...
La relación que aparece entre `technicity` y `school` se fundamenta en que colegios donde hay mayor porcentaje de estudiantes **afam** también lo poseen en profesores (esto lo vemos en la próxima Figura, recordando que en **inner-city** hay una gran cantidad de estudiantes con `ethnicity = "afam"`). La alta asociación entre **school** y **system** se explica a partir del hecho de que escuelas del mismo tipo pueden pertenecer al mismo sistema escolar.
...
El modelo causal que se muestra a continuación es el que ha dado menos problemas a la hora de recuperar efectos del tratamiento en etapas posteriores (sobre todo en la parte de DoubleML). Son también las variables sobre las que más hincapié se hace en la literatura que hemos consultado [@hanck2021introduction; @Mosteller].
...
Veremos posteriormente que la variable `schoolid` absorbe mucha de la variabilidad explicada en un modelo de regresión múltiple, pero resulta problemática en estimaciones de ATE con otro tipo de modelo y por ello no la incluimos en el modelo DAG. Al resto de variables como `tethnicity`, `degree` o `ladder` les sucedía algo parecido, pero en este caso tampoco aportaban tanto a la variabilidad explicada en el modelo de regresión.


```{r DAGmodel, fig.cap=capFig("Modelo DAG para la recuperación del efecto del tratamiento. Consideramos las variables son de confusión.", y = "DAGmodel")}
dag <- dagify(
...
Si asumimos que ha habido fallos en el protocolo de asignación aleatoria del tratamiento, entonces la condición de independencia de los resultdos potenciales con el estatus de tratamiento no se cumple y el estimador diferencia de medias es un estimador sesgado. No obstante, podemos considerar que las diferencias en el estatus de tratamiento emergen por diferencias en las variables de confusión X, anteriormente mencionadas, y que una vez controladas la independencia se restablece (independencia condicional).
...
El matching es un método no paramétrico de pre-tratamiento de la muestra que imputa el resultado potencial no observado de una unidad utilizando resultados observables de unidades similares (en variables de confusión) con estatus de tratamiento opuesto. Esta técnica puede utilizarse para aumentar la eficiencia de la estimación del efecto causal cuando se tiene una base de datos rica en características observables, si bien no permite eliminar el sesgo que emerge consecuencia de variables de confusión no observadas.

En primer lugar, para poder aplicar el matching, debemos trabajar sin datos faltantes. En este caso, eliminaremos los registros que contengan `NA` (tras separar en 4 conjuntos de datos, uno por cada curso).

```{r match-df-year}
starDf_k <- starDf %>%
              select(gender, ethnicity_grouped, stark, markK, lunchk, experiencek, schoolk) %>%
              filter_all(all_vars(!is.na(.)))%>%
              filter(stark != "regular+aide")%>%
              mutate(stark = fct_drop(stark))
starDf_1 <- starDf %>%
              select(gender, ethnicity_grouped, star1, mark1, lunch1, experience1, school1) %>%
              filter_all(all_vars(!is.na(.)))%>%
              filter(star1 != "regular+aide")%>%
              mutate(star1 = fct_drop(star1))
starDf_2 <- starDf %>%
              select(gender, ethnicity_grouped, star2, mark2, lunch2, experience2, school2) %>%
              filter_all(all_vars(!is.na(.)))%>%
              filter(star2 != "regular+aide")%>%
              mutate(star2 = fct_drop(star2))
starDf_3 <- starDf %>%
              select(gender, ethnicity_grouped, star3, mark3, lunch3, experience3, school3) %>%
              filter_all(all_vars(!is.na(.)))%>%
              filter(star3 != "regular+aide")%>%
              mutate(star3 = fct_drop(star3))
```
...
```{r match-object}
star_k_match_object <- matchit(stark ~ experiencek + lunchk + schoolk + gender + ethnicity_grouped,
                                method="full",
                                distance="glm",
                                data=starDf_k,
                                estimand = "ATE")
star_1_match_object <- matchit(star1 ~ experience1 + lunch1 + school1 + gender + ethnicity_grouped,
                                method="full",
                                distance="glm",
                                data=starDf_1,
                                estimand = "ATE")
star_2_match_object <- matchit(star2 ~ experience2 + lunch2 + school2 + gender + ethnicity_grouped,
                                method="full",
                                distance="glm",
                                data=starDf_2,
                                estimand = "ATE")
star_3_match_object <- matchit(star3 ~ experience3 + lunch3 + school3 + gender + ethnicity_grouped,
                                method="full",
                                distance="glm",
                                data=starDf_3,
                                estimand = "ATE")
...
```{r match-love-plot-kindergarden, fig.cap=capFig("Love-plot de kindergarten", y = "match-love-plot-kindergarden")}

love.plot(star_k_match_object,stats = c("m","var","ks"),
            abs = TRUE,thresholds=c(m=0.1,ks=0.05,var=2),
            stars = "std")
```

```{r match-love-plot-grade1, fig.cap=capFig("Love-plot del primer curso.", y = "match-love-plot-grade1")}
love.plot(star_1_match_object,stats = c("m","var","ks"),
            abs = TRUE,thresholds=c(m=0.1,ks=0.05,var=2),
            stars = "std")
```

```{r match-love-plot-grade2, fig.cap=capFig("Love-plot del segundo curso.", y = "match-love-plot-grade2")}

love.plot(star_2_match_object,stats = c("m","var","ks"),
            abs = TRUE,thresholds=c(m=0.1,ks=0.05,var=2),
            stars = "std")
```

```{r match-love-plot-grade3, fig.cap=capFig("Love-plot del tercer curso.", y = "match-love-plot-grade3")}
love.plot(star_3_match_object,stats = c("m","var","ks"),
            abs = TRUE,thresholds=c(m=0.1,ks=0.05,var=2),
            stars = "std")
```
...
```{r match-data}
star_k_match_data <- match.data(star_k_match_object)
star_1_match_data <- match.data(star_1_match_object)
star_2_match_data <- match.data(star_2_match_object)
star_3_match_data <- match.data(star_3_match_object)
```


```{r match-meandiff-kindergarden, message = FALSE, include = FALSE}
modelo_match_k <- glm(markK ~ stark*(experiencek + lunchk + schoolk + gender +  ethnicity_grouped),
                      data=star_k_match_data,family = gaussian(),
                      weights = weights)

pred.gformula <- predictions(modelo_match_k,
                      variables = "stark", # variable para la que generar contrafactuales
                      vcov = ~subclass,   # estimacion robusta de se
                      newdata = star_k_match_data, # ATE
                      wts = "weights",
                      by="stark") # variable de agregación

avgcompk <- avg_comparisons(modelo_match_k,variables = "stark",
                      vcov = ~subclass,   # estimacion robusta de se
                      newdata = star_k_match_data,
                      wts = "weights")
```


```{r match-meandiff-grade1, message = FALSE, include = FALSE}
modelo_match_1 <- glm(mark1 ~ star1*(experience1 + lunch1 + school1 + gender +  ethnicity_grouped),
                      data=star_1_match_data,family = gaussian(),
                      weights = weights)

pred.gformula <- predictions(modelo_match_1,
                      variables = "star1", # variable para la que generar contrafactuales
                      vcov = ~subclass,   # estimacion robusta de se
                      newdata = star_1_match_data, # ATE
                      wts = "weights",
                      by="star1") # variable de agregación


avgcomp1 <- avg_comparisons(modelo_match_1,variables = "star1",
                      vcov = ~subclass,   # estimacion robusta de se
                      newdata = star_1_match_data,
                      wts = "weights")

```


```{r match-meandiff-grade2, message = FALSE, include = FALSE}
modelo_match_2 <- glm(mark2 ~ star2*(experience2 + lunch2 + school2 + gender +  ethnicity_grouped),
                      data=star_2_match_data,family = gaussian(),
                      weights = weights)
pred.gformula <- predictions(modelo_match_2,
                      variables = "star2", # variable para la que generar contrafactuales
                      vcov = ~subclass,   # estimacion robusta de se
                      newdata = star_2_match_data, # ATE
                      wts = "weights",
                      by="star2") # variable de agregación

avgcomp2 <- avg_comparisons(modelo_match_2,variables = "star2",
                      vcov = ~subclass,   # estimacion robusta de se
                      newdata = star_2_match_data,
                      wts = "weights")
```


```{r match-meandiff-grade3, message = FALSE, include = FALSE}
modelo_match_3 <- glm(mark3 ~ star3*(experience3 + lunch3 + school3 + gender +  ethnicity_grouped),
                      data=star_3_match_data,family = gaussian(),
                      weights = weights)

pred.gformula <- predictions(modelo_match_3,
                      variables = "star3", # variable para la que generar contrafactuales
                      vcov = ~subclass,   # estimacion robusta de se
                      newdata = star_3_match_data, # ATE
                      wts = "weights",
                      by="star3") # variable de agregación

avgcomp3 <- avg_comparisons(modelo_match_3,variables = "star3",
                      vcov = ~subclass,   # estimacion robusta de se
                      newdata = star_3_match_data,
                      wts = "weights")
```

```{r avgCompShowing}
# Poner en una tabla los resultados avgcompk, avgcomp1, avgcomp2, avgcomp3 (luego hacer kable)
avgcomp <- rbind(avgcompk, avgcomp1, avgcomp2, avgcomp3)
rownames(avgcomp) <- c("K", "1", "2", "3")
colnames(avgcomp) <- c("Term", "Contrast", "Estimate", "Std. Error", "z", "Pr(>|z|)", "S", "2.5 %", "97.5%")

kable(avgcomp, caption = capTab("Estimaciones del efecto del tratamiento para los diferentes cursos tras el matching"))
```
...
axs[1, 1].set_title('Tercer Curso')
...
![<strong>Figura 36. </strong> Distribución de la respuesta por estatus de tratamiento (clase regular vs clase reducida) en los diferentes cursos.](./images/boxplotRespuestaTratamiento.png)
...
```{python TE-SLearner, fig.cap = "<strong> Figura 37. </strong> Histograma de efectos de tratamiento estimados por el S-Learner en los diferentes cursos.", echo = FALSE, message = FALSE}
fig, axs = plt.subplots(2, 2, figsize=(14, 10))
...
```{python CI-SLearner, fig.cap = "<strong> Figura 38. </strong> Estimaciones de efectos de tratamiento y bandas de confianza en los diferentes cursos.", echo = FALSE, message = FALSE}
fig, axs = plt.subplots(2, 2, figsize=(14, 10))
...
title = r'$\bf{Tabla\ 7.}$ Intervalos de confianza para el ATE en los diferentes cursos usando el S-Learner.'
plt.title(title, fontsize=16, pad=1)
...
![](./images/ate_intervals_slearner.png)
...
![<strong>Figura 39. </strong> Gráfico de waterfall que indica cómo se da la combinación para la predicción final (Kindergarten) (Para la primera muestra).](./images/wfallSLearner_k.png)
...
![<strong>Figura 40. </strong> Gráfico de barras con la importancia de cada característica (Kindergarten).](./images/barSLearner_k.png)
...
for Y, T, X in zip(Y_list, T_list, X_list):
    ...
    if first_iteration_x:
        shap_values_x = X_learner.shap_values(X)
...
```{python, fig.cap = "<strong> Figura 41. </strong> Histograma de efectos de tratamiento estimados por el X-Learner en los diferentes cursos.", echo = FALSE, message = FALSE}
...
```{python, fig.cap = "<strong> Figura 42. </strong> Estimaciones de efectos de tratamiento y bandas de confianza en los diferentes cursos (XLearner).", echo = FALSE, message = FALSE}
# Crear figura y subplots
fig, axs = plt.subplots(2, 2, figsize=(14, 10))
...
title = r'$\bf{Tabla\ 8.}$ Intervalos de confianza para el ATE en los diferentes cursos usando el X-Learner.'
plt.title(title, fontsize=16, pad=1)
...
![](./images/ate_intervals_xlearner.png)
...
![<strong>Figura 43. </strong> Gráfico de waterfall que indica cómo se da la combinación para la predicción final (Kindergarten) (Para la primera muestra).](./images/wfallXLearner_k.png)
...
![<strong>Figura 44. </strong> Gráfico de barras con la importancia de cada característica (Kindergarten).](./images/barXLearner_k.png)



## Uso de DoubleML

```{python doubleml-import}
...
En el código se ha creado una función llamada `run_dml_analysis` que utiliza la biblioteca DoubleML para realizar un análisis causal. Se han empleado los modelos de `RandomForest` y `XGB` tanto en su versión de *Regresor* como *Clasificador* (según queramos sacar la media condicional de respuesta o el propensity score, respectivamente).
Utilizando estos modelos, se ajustan dos tipos de modelos `DoubleMLPLR` (*Double ML with Partially Linear Regression*), uno para cada algoritmo de aprendizaje automático. Luego, se generan resúmenes de los resultados y, si se proporcionan grupos, se calculan los efectos condicionales (GATE) para cada modelo y se visualizan mediante gráficos de barras con intervalos de confianza. 

```{python run_dml_analysis-function}
def run_dml_analysis(data, y_col, d_cols, ml_l_rf, ml_l_xgb, ml_m_rf, ml_m_xgb, seed = seed, varGATE = None, grupos=None, curso=None, n_folds=10, show_summary=True):
    ...
    ml_l=ml_l_rf, # para sacar la media condicional de respuesta
    ml_m=ml_m_rf, # para sacar el propensity score
    n_folds=n_folds)
    dml_plr_rf.fit()
    ...
    # Graficar barras de error para RandomForest
      ...
      axs[0].set_title('GATEs RandomForest ' + curso + '\nPor ' + varGATE, fontsize=titleFontSize)
    ...
    # Graficar barras de error para XGBoost
      ...
      axs[1].set_title('GATEs XGBoost ' + curso + '\nPor ' + varGATE, fontsize=titleFontSize)
...
```{python ate-summary-doubleml, include = FALSE}
...
```{python ate-summary-doubleml-table, include = FALSE, message = FALSE}
summaries = {
    "Curso": ["Kinder", "Kinder", "1º Curso", "1º Curso", "2º Curso", "2º Curso", "3º Curso", "3º Curso"],
    "Modelo": ["RF", "XGBoost", "RF", "XGBoost", "RF", "XGB", "RF", "XGB"],
    "Coef": [
        dml_plr_rf_k.summary['coef'][0], dml_plr_xgb_k.summary['coef'][0],
        dml_plr_rf_1.summary['coef'][0], dml_plr_xgb_1.summary['coef'][0],
        dml_plr_rf_2.summary['coef'][0], dml_plr_xgb_2.summary['coef'][0],
        dml_plr_rf_3.summary['coef'][0], dml_plr_xgb_3.summary['coef'][0]
    ],
    "Std Err": [
        dml_plr_rf_k.summary['std err'][0], dml_plr_xgb_k.summary['std err'][0],
        dml_plr_rf_1.summary['std err'][0], dml_plr_xgb_1.summary['std err'][0],
        dml_plr_rf_2.summary['std err'][0], dml_plr_xgb_2.summary['std err'][0],
        dml_plr_rf_3.summary['std err'][0], dml_plr_xgb_3.summary['std err'][0]
    ],
    "t": [
        dml_plr_rf_k.summary['t'][0], dml_plr_xgb_k.summary['t'][0],
        dml_plr_rf_1.summary['t'][0], dml_plr_xgb_1.summary['t'][0],
        dml_plr_rf_2.summary['t'][0], dml_plr_xgb_2.summary['t'][0],
        dml_plr_rf_3.summary['t'][0], dml_plr_xgb_3.summary['t'][0]
    ],
    "P>|t|": [
        dml_plr_rf_k.summary['P>|t|'][0], dml_plr_xgb_k.summary['P>|t|'][0],
        dml_plr_rf_1.summary['P>|t|'][0], dml_plr_xgb_1.summary['P>|t|'][0],
        dml_plr_rf_2.summary['P>|t|'][0], dml_plr_xgb_2.summary['P>|t|'][0],
        dml_plr_rf_3.summary['P>|t|'][0], dml_plr_xgb_3.summary['P>|t|'][0]
    ],
    "2.5 %": [
        dml_plr_rf_k.summary['2.5 %'][0], dml_plr_xgb_k.summary['2.5 %'][0],
        dml_plr_rf_1.summary['2.5 %'][0], dml_plr_xgb_1.summary['2.5 %'][0],
        dml_plr_rf_2.summary['2.5 %'][0], dml_plr_xgb_2.summary['2.5 %'][0],
        dml_plr_rf_3.summary['2.5 %'][0], dml_plr_xgb_3.summary['2.5 %'][0]
    ],
    "97.5 %": [
        dml_plr_rf_k.summary['97.5 %'][0], dml_plr_xgb_k.summary['97.5 %'][0],
        dml_plr_rf_1.summary['97.5 %'][0], dml_plr_xgb_1.summary['97.5 %'][0],
        dml_plr_rf_2.summary['97.5 %'][0], dml_plr_xgb_2.summary['97.5 %'][0],
        dml_plr_rf_3.summary['97.5 %'][0], dml_plr_xgb_3.summary['97.5 %'][0]
    ]
}
...
```{python showingSummary_df, include = FALSE}
# Formatear el DataFrame para mostrar solo dos decimales
summary_df_formatted = summary_df.round(3)

# Crear la figura y los ejes
fig, ax = plt.subplots(figsize=(12, 3))  # Puedes ajustar el tamaño de la figura aquí

ax.axis('off')
ax.axis('tight')

table = ax.table(cellText=summary_df_formatted.values, colLabels=summary_df_formatted.columns, cellLoc='center', loc='center')

# Estilizar la tabla
table.auto_set_font_size(False)
table.set_fontsize(12)
table.scale(1, 1)  # Puedes ajustar la escala aquí

title = r'$\bf{Tabla\ 9.}$ Resultados obtenidos para el ATE con DoubleML.'
plt.title(title, fontsize=16, pad=1)
fig.tight_layout()

plt.savefig("./images/ate_summary_doubleml.png", bbox_inches='tight', dpi=300)

plt.show()
```

![](./images/ate_summary_doubleml.png)
...
```{python, fig.cap = "<strong> Figura 45. </strong> Efecto del tratamiento por etnia en los diferentes cursos.", echo = FALSE, message = FALSE}
dml_plr_rf_3, dml_plr_xgb_3 = run_dml_analysis(data=starDf_3,
                                           y_col='mark3',
                                           d_cols='star3',
                                           ml_l_rf=ml_m_rf,
...
```{python, fig.cap = "<strong> Figura 46. </strong> Efecto del tratamiento por género en los diferentes cursos.", echo = FALSE, message = FALSE}
dml_plr_rf_3, dml_plr_xgb_3 = run_dml_analysis(data=starDf_3,
                                           y_col='mark3',
                                           d_cols='star3',
                                           ml_l_rf=ml_m_rf,
...
```{python, fig.cap = "<strong> Figura 47. </strong> Efecto del tratamiento por tipo de escuela en los diferentes cursos.", echo = FALSE, message = FALSE}
dml_plr_rf_3, dml_plr_xgb_3 = run_dml_analysis(data=starDf_3,
                                           y_col='mark3',
                                           d_cols='star3',
                                           ml_l_rf=ml_m_rf,
...
En este trabajo hemos realizado un análisis en profundidad del proyecto *STAR*. Mediante el análisis exploratorio de las diferentes características del estudio, hemos observado diferentes relaciones entre variables que se han manifestado en resultados posteriores.

En primer lugar, hemos escogido como efecto la suma de las puntuaciones de matemáticas y lectura de los estudiantes, siendo el tratamiento la pertenencia a una clase normal o a una de tamaño reducido o normal con ayuda. Además, hemos estudiado cada uno de los cuatro cursos por separado,

A partir del primer análisis, hemos desarrollado un DAG adecuado con cinco variables confusoras: género del estudiante, experiencia del profesorado, etnicidad del estudiante, participación en el almuerzo gratuito y tipo de escuela.

Para estimar el efecto del tratamiento, hemos empleado primeramente el estimador naive diferencia de medias y modelos de regresión. Los resultados obtenidos indican que la pertenencia a clases reducidas tiene un efecto positivo sobre las puntuaciones de los estudiantes. Además, la adición de otras variables al modelo no provoca variaciones significativas en los resultados, lo que reafirma la consistencia de los resultados. Por otro lado, la pertenencia a clases normales con ayuda no tiene efecto significativo.

A fin de asegurar la independencia condicional, hemos aplicado técnicas de *matching*, que han reducido las diferencias entre las distribuciones de tratados y no tratados. En consecuencia, hemos observado una disminución del ATE.

Una vez hemos obtenido los datos emparejados, hemos utilizado Meta-Learners y DoubleML para estimar el efecto. El S-Learner ofrece un ATE ligeramente inferior a los resultados de los modelos de regresión, en todo caso compatibles con su intervalo de confianza; mientras que los valores resultantes del X-Learner son de peor calidad, ya que, aunque el valor medio del ATE es similar, el intervalo de confianza es considerablemente más amplio. Por otro lado, DoubleML indica que el efecto es estadísticamente significativo, y mayor para el Primer Curso que para el resto de cursos (quizás esto se deba a un resultado deficiente en dicho año por parte de la clase regular, como se comentó en la sección de `Modelos de Regresión`).

También hemos estimado los efectos heterogéneos con el GATE en función de la etnicidad, el género y el tipo de escuela. En el primero, los grupos no caucásicos parecen beneficiarse de un efecto mayor en Kindergarten, mientras que el resto de grupos y cursos no presentan diferencias significativas. El GATE de género tampoco es estadísticamente significativo. Por último, el tipo de escuela sí ha resultado relevante, siendo beneficiados diferentes grupos en función del curso: en Kindergarten, inner-city y urbanos; en 1º, suburbano; y en 2º y 3º, inner-city, aunque menos relevante.

Por tanto, el estudio de los datos del proyecto *STAR* nos permite afirmar que la pertenencia a clases de tamaño reducido resulta en un efecto positivo sobre las puntuaciones de los estudiantes en todos los cursos, especialmente en el Primer Curso, respecto a las clases normales. Además, observamos la existencia de algunos grupos heterogéneos para las variables confusoras más relevantes que implican que ciertos grupos de estudiantes se vieron beneficiados por un mayor efecto.
