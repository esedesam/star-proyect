---
title: "Proyecto STAR"
subtitle: "Inferencia Causal. Máster en Ciencia de Datos (UV)"
...
plotNumericFeature <- function(starDf, featureId, yLabel = NA) {
  if (is.na(yLabel)) {
    yLabel <- featureId
  }
  plotDf <- pivot_longer(starDf, cols = starts_with(featureId), names_to = "grade", values_to = "readScore")
  ...
  ggplot(plotDf, aes(x = grade, y = readScore)) +
    geom_boxplot(fill = "steelblue") +
    labs(x = "Curso", y = yLabel)
}
```
...
```{r}
ggplot(STAR, aes(x = gender)) +
  geom_bar()
```

```{r}
plotNumericFeature(starDf, "read", "Puntuación de lectura")
plotNumericFeature(starDf, "math", "Puntuación de matemáticas")
plotNumericFeature(starDf, "experience", "Experiencia del maestro (años)")
```
