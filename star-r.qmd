...
Vamos a realizar técnicas de Match IT, una metodología utilizada para mejorar la precisión y eficiencia en la correspondencia de datos. La implementación de Match IT es crucial para garantizar la integridad y calidad de la información en diversos sistemas, facilitando la identificación y consolidación de registros duplicados o inconsistentes. Al aplicar estas técnicas, se optimizan procesos de gestión de datos, lo que resulta en decisiones más informadas y una administración más eficaz de los recursos.

Si asumimos que existen variables confusoras en la estimación del ATE, se reqquiere el balanceo de los datos y qu epara una de las variables haya una probabilidad entre 0 y 1 de recibir el tratamiento. Realizamos el mathing e imputamos la respuesta potencial que falta de cada variable (contrafactual).

En primer lugar, para poder aplicar el método de Match IT, debemos trabajar sin datos faltantes. En este caso, eliminaremos los registros que contengan NAs.
...
Una vez obtenidos los Data Frames limpios, realicamos el Match It con las variables: experience, lunch, school, gender y ethnicity_grouped.
...
Comprobamos el equlibrio de las variables tras el Match It visualizando los love plots de los distintos cursos.

```{r match-love-plot-kindergarden, fig.cap=capFig("Love-plot del curso kindergarden.", y = "match-love-plot-kindergarden")}
...
```{r match-love-plot-grade1, fig.cap=capFig("Love-plot del curso grade 1.", y = "match-love-plot-grade1")}
love.plot(star_1_match_object,stats = c("m","var","ks"),
            abs = TRUE,thresholds=c(m=0.1,ks=0.05,var=2),
            stars = "std")
```

```{r match-love-plot-grade2, fig.cap=capFig("Love-plot del curso grade 2.", y = "match-love-plot-grade2")}
...
```{r match-love-plot-grade3, fig.cap=capFig("Love-plot del curso grade 3.", y = "match-love-plot-grade3")}
...
Por último, obtenemos los datos correspondientes al proceso de Match It, para poder recuperar el ATE.
...
Recuperación del ATE para el curso kindergarden:

```{r match-meandiff-kindergarden, message = FALSE}
...
avg_comparisons(modelo_match_k,variables = "stark",
                      vcov = ~subclass,   # estimacion robusta de se
                      newdata = star_k_match_data,
                      wts = "weights")
```

Recuperación del ATE para el curso grade 1:

```{r match-meandiff-grade1, message = FALSE}
...
avg_comparisons(modelo_match_1,variables = "star1",
                      vcov = ~subclass,   # estimacion robusta de se
...
Recuperación del ATE para el curso grade 2:

```{r match-meandiff-grade2, message = FALSE}
...
avg_comparisons(modelo_match_2,variables = "star2",
                      vcov = ~subclass,   # estimacion robusta de se
                      newdata = star_2_match_data,
                      wts = "weights")
```

Recuperación del ATE para el curso grade 3:

```{r match-meandiff-grade3, message = FALSE}
...
avg_comparisons(modelo_match_3,variables = "star3",
                      vcov = ~subclass,   # estimacion robusta de se
                      newdata = star_3_match_data,
                      wts = "weights")
```

Observamos que, en todos los casos, el efecto causal se reduce respecto a la previa obtención del ATE con los modelos lineales sin usar Match It. Esta ligera reducción puede deberse a la consideración de variables de confusión en el tratameinto. Dado que está mejor identificado el efecto causal, esta estimación debería ser más fiable.
...
title = r'$\bf{Tabla\ 32.}$ Resultados obtenidos para el ATE con DoubleML.'
...
dml_plr_rf_2, dml_plr_xgb_2 = run_dml_analysis(data=starDf_2,
                                           ...
                                           n_folds=10)
```
```{python, fig.cap = "<strong> Figura 43. </strong> Efecto del tratamiento por etnia en los diferentes cursos.", echo = FALSE, message = FALSE}
dml_plr_rf_3, dml_plr_xgb_3 = run_dml_analysis(data=starDf_3,
                                           y_col='mark3',
                                           d_cols='star3',
                                           ml_l_rf=ml_m_rf,
...
dml_plr_rf_2, dml_plr_xgb_2 = run_dml_analysis(data=starDf_2,
                                           ...
                                           n_folds=10)
```
```{python, fig.cap = "<strong> Figura 44. </strong> Efecto del tratamiento por género en los diferentes cursos.", echo = FALSE, message = FALSE}
dml_plr_rf_3, dml_plr_xgb_3 = run_dml_analysis(data=starDf_3,
                                           y_col='mark3',
                                           d_cols='star3',
                                           ml_l_rf=ml_m_rf,
...
```
```{python, fig.cap = "<strong> Figura 45. </strong> Efecto del tratamiento por tipo de escuela en los diferentes cursos.", echo = FALSE, message = FALSE}
dml_plr_rf_3, dml_plr_xgb_3 = run_dml_analysis(data=starDf_3,
                                           y_col='mark3',
                                           d_cols='star3',
                                           ml_l_rf=ml_m_rf,
