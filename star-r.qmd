...
p_load(AER, ggplot2, ggthemes, dplyr, tidyr, knitr, stargazer, sandwich, fastDummies, MatchIt)
...
```{python}
# Necesito poner las notas por cuartiles

```

```{python}
def preprocess_data(df, star_column, experience_column, school_column, lunch_column, ethnicity_column, mark_column):
    
    # Seleccionar columnas relevantes y eliminar filas con valores faltantes
    df_processed = df[[star_column, experience_column, school_column, 'gender', lunch_column, ethnicity_column, mark_column]].dropna()
    ...
    # Crear variables dummy para la columna 'school'
...
```{python}
starDf_k = preprocess_data(starDf, "stark", "experiencek", "schoolk", "lunchk", "ethnicity_grouped", "markK")
...
Xk = starDf_k.drop(columns=["markK", "stark"]).values
Tk = starDf_k["stark"].values

Y1 = starDf_1["mark1"].values
X1 = starDf_1.drop(columns=["mark1", "star1"]).values
T1 = starDf_1["star1"].values

Y2 = starDf_2["mark2"].values
X2 = starDf_2.drop(columns=["mark2", "star2"]).values
T2 = starDf_2["star2"].values

Y3 = starDf_3["mark3"].values
X3 = starDf_3.drop(columns=["mark3", "star3"]).values
...
```{python}
dml_data = DoubleMLData(starDf_k, 
                        y_col='markK', 
                        d_cols='stark', 
                        x_cols=None)
```
...
plt.ylim([10, 25]) # cambiar límites manualmente de visualización
...
plt.show()
```
### Efectos condicionales (GATE)

```{python}
grupos = pd.DataFrame(
    np.column_stack((starDf_k['ethnicity_grouped'] == 1,
                     starDf_k['ethnicity_grouped'] == 0)),
    columns=['cauc', 'afam+other'])
print(grupos.head())
```
```{python}
gate = dml_plr_xgb.gate(groups = grupos)
print(gate.confint(level = 0.95))

ci = gate.confint(level=0.95, joint=False)# joint = False si estamos más interesados en variabilidad de efectos para cada grupo por separado
```
```{python}
...
*Discutir resultados observados* Para Kindergarten se ve claramente que el efecto es muy superior para los no caucásicos (tal y como se mencionó en el artículo que pasó Jesús :D.)