...
p_load(AER, ggplot2, ggthemes, dplyr, tidyr, knitr, stargazer, sandwich, GGally, stringr, vcd, MatchIt, cobalt, marginaleffects, forcats)
...
## Selección de variables confusoras

Con la finalidad de diseñar un modelo causal, a continuación vamos a comprobar qué variables son más relevantes en un modelo de regresión lineal múltiple que prediga la respuesta al tratamiento, ya que las variables más relevantes pueden ser candidatas a variables confusoras del modelo causal, o explicar parte de la variabilidad de la respuesta.

Para determinar si estas variables son significativas en la predicción la función `lm()` realiza un test t-Student sobre cada uno de los coeficientes de la regresión. Puesto que vamos a realizar un análisis independiente para cada curso escolar, planteamos cuatro modelos distintos, uno para cada curso, donde tanto las variables como la respuesta seleccionadas se corresponden a su curso correspondiente.
...
Crearemos en primer lugar una nueva variable que sea la suma de las puntuaciones de lectura y matemáticas para cada curso. Además, calcularemos el percentil de esta variable para cada curso.
...
starDf_k <- starDf %>%
  select(gender, ethnicity_grouped, stark, markK, lunchk, experiencek, schoolk, schoolidk, tethnicityk) # tethnicity hard to be used
...
html_comparisonModels <- stargazer(
  ...
  title = capTab("Estimación de diferencias: comparación entre modelos"),
  header = FALSE, 
  model.numbers = FALSE,
...
kable(summaryBasicModel, caption = capTab("Estimaciones de efecto del tratamiento normalizadas por desviación estándar muestral"))
```
...
```{python }
print(Tk.mean(), T1.mean(), T2.mean(), T3.mean())
```
...
y0 = yTrain_k[tTrain_k==0]
y1 = yTrain_k[tTrain_k==1]
respuesta=[y0,y1]
etiquetas = ["control","tratados"]

plt.figure(figsize=(8, 6))  # Opcional: Define el tamaño de la figura
plt.boxplot(respuesta, labels=etiquetas)

plt.title('Distribución de la respuesta por estatus de tratamiento')
plt.ylabel('Valor de la respuesta')
...
```{python}
# Estimación efecto por diferencia de medias (sesgado)
y1.mean() - y0.mean()
```
...
#overall_model = make_pipeline(poly, ridge_reg)
...
S_learner.fit(yTrain_k, tTrain_k, X=xTrain_k, inference="bootstrap")

# Estimamos efectos tratamiento sobre datos de prueba
S_te = S_learner.effect(xTest_k)

# Histograma de TEs 
plt.figure(figsize=(8, 6))
plt.hist(S_te, bins=30, color='blue', alpha=0.7)
plt.title('Histograma de efectos de tratamiento estimados')
plt.xlabel('Efecto de tratamiento')
plt.ylabel('Frecuencia')
plt.show()
```

```{python}
lb, ub = S_learner.effect_interval(xTest_k)
...
```{python}
S_learner.ate_interval(xTest_k, T0=0, T1=1)
```


```{python}
shap_values = S_learner.shap_values(xTest_k)

plt.figure(figsize=(20, 8))
shap.plots.waterfall(shap_values['Y0']['T0_1.0'][1], max_display=20, show=False)
...
![Test](./images/wfallSLearner_k.png)
...
![Testing](./images/barSLearner_k.png)
...
```{python}
n = 10000
# Instancia de X learner
models = GradientBoostingRegressor(n_estimators=100, max_depth=10, min_samples_leaf=int(n/100))
propensity_model = RandomForestClassifier(n_estimators=100, max_depth=6, 
                                                  min_samples_leaf=int(n/100))
                                                  
X_learner = XLearner(models=models, propensity_model=propensity_model)
# Train X_learner
X_learner.fit(yTrain_k, tTrain_k, X=xTrain_k,inference="bootstrap")
# Estimate treatment effects on test data
X_te = X_learner.effect(xTest_k)

# Histograma de TEs 
plt.figure(figsize=(8, 6))
plt.hist(X_te, bins=30, color='blue', alpha=0.7)
plt.title('Histograma de efectos de tratamiento estimados')
plt.xlabel('Efecto de tratamiento')
plt.ylabel('Frecuencia')
plt.show()
...
DoubleML es una técnica de estimación de efectos causales, utilizando tecnicas de Machine Learning en el proceso. Además de estimar efectos causales, doubleML también permite calcular intervalos de confianza de esos estimadores, estos estimadores son insesgados, ya que se utilizan técnicas de ortogonalización. Un estimador de ML naive produce estimaciones sesgadas. Nos interesa utilizar ML por su gran potencia y capacidad de generar no linealidades.

Partimos de un modelo parcialmente linear, con una variable tratamiento (D), otra, la variable respuesta (Y) y variables de confusión (Z).

Double ML funciona básicamente de la siguiente manera:

1. Predecir D basado en Z usando Machine Learning
2. Predecir Y basado en Z usando Machine Learning
3. Regresión lineal de los residuos de 2 sobre los residuos de 1, para obtener una estimación de θ₀

Finalmente utilizamos cross-fitting luego para reducir el overfitting del modelo a la hora de estimar las funciones. nuisance
...
Parece que los grupos no caucásicos se benefician en mayor medida del hecho de estar en clases más reducidas, el único curso donde el efecto tratamiento no muestra diferencias significativas es en el primer curso, como se observa en el gráfico.

GENERO:

En este caso observamos que las hombres se habrían beneficiado más del tratamiento que las hombres, sin embargo en kindergarten, que es cuando los niños entran en el sistema educativo, serían los hombres los que se beneficiarian más que las mujeres. En el resto de cursos son las mujeres las que perciben mayor efecto tratamiento.
